...
class ProcessingPipeline:
    ...
    def load_model(self):
        ...
        model_name = self.config.get("model", "animeganv2")
        variant = self.config.get("model_variant")
        weights_dir = self.config.get("models_directory", "models/weights")

        logger.info(f"Loading model: {model_name}")
        self.model = create_model(model_name, self.device, variant, weights_dir)
        self.model.load()
    ...
    def _generate_report(self, job_id: str, metadata: dict, 
        ...
        report = {
            ...
            "processing_start_time": metadata.get("started_at", ""),
